...
| Image Provenance | **Cosign** | Signs images and verifies they came from a trusted build path.[7] |
| Software Bill of Materials (SBOM) | **Syft** | Generates SBOM to understand image contents and dependencies.[8] |
| Runtime Security Monitoring | **Falco** | Detects suspicious activity and policy violations at runtime.[9] |
...
| Secrets management | No secrets in repo, server-side environment files (or AWS Secrets Manager), GitHub secrets |
| Build security | Pinned actions, reproducible image builds, artifact attestations, SBOM generation |
| Container security | Minimal images, non-root user, image scanning, health checks, image signing |
...
| Image Signing | Cosign | Verifies image authenticity and integrity.[7] |
| SBOM Generation | Syft | Generates a Software Bill of Materials for image transparency.[8] |
| Runtime Security | Falco | Detects anomalous behavior and threats on the host.[9] |
...
GitHub’s documentation shows that GitHub Actions can build and publish Docker images, log into `ghcr.io`, and generate artifact attestations for image provenance.[4] Your pipeline should use that capability, but keep the workflow modular.
...
| Merge to main | Build images, run Trivy, generate SBOM, publish image, sign image | Prevents vulnerable or untrusted images from shipping |
...
Your application images should run as non-root users, use minimal base images where practical, set explicit health checks, and avoid baking secrets into images. Trivy should scan built images before deployment.[5] Additionally, **Cosign** should be used to sign your container images, providing cryptographic assurance of their origin and integrity.[7]

### 6. Secure supply chain

GitHub explicitly recommends pinning actions to a **commit SHA**, rather than a mutable tag, because tags or branches may change without warning.[4] That single control is often missed by beginners and is a valuable mark of maturity. Furthermore, generating a **Software Bill of Materials (SBOM) with Syft** provides transparency into the components and dependencies within your container images, enhancing supply chain security.[8]
...
A lightweight DAST pass with OWASP ZAP baseline against your staging URL should check the externally reachable application after deployment. This is not a replacement for manual testing, but it closes an important gap in many beginner pipelines. For continuous runtime security, **Falco** should be deployed on the EC2 instance to detect suspicious activity and policy violations within your containers and on the host itself.[9]
...
GitHub’s workflow examples document logging into `ghcr.io`, building and pushing images, and generating an artifact attestation.[4] Implement this next so that your deployment consumes immutable, pre-scanned images from the registry. Additionally, integrate **Syft for SBOM generation** and **Cosign for image signing**.[7][8]
...
| CI | Tests, linting, Semgrep, Gitleaks, Hadolint, Syft SBOM generation |
| Container security | Trivy image scan before release, Cosign image signing |
| CD | Tag-based deployment from registry |
| Access security | Key-only SSH, restricted port 22, AWS Secrets Manager integration |
| Documentation | README, architecture, threat model, runbook |
| Recovery | Backup script and rollback script |
| Runtime Security | Falco for host and container runtime monitoring |
...
|---|---|---|
| Secret leakage | API key committed to repo | Gitleaks, secret review, `.env.example` only, AWS Secrets Manager |
| Vulnerable image | Outdated OS packages in image | Trivy gating, Syft SBOM for transparency |
...
| Runtime compromise | Container escape or suspicious process | Falco runtime monitoring |
| Untrusted image | Image not from a trusted source | Cosign image signing and verification |
